...
// Python admission binds real task receipts; this service accepts only those prebound targets.
@main struct BrokerMain {
 static func finish(_ item:CaptureItem) async -> String? {
  var problem:String?
  do {try item.presence.finish()} catch {problem=String(describing:error)}
  if let captureError=item.queue.sync(execute:{item.recorder.error}) {problem=problem ?? captureError}
  ...
  emit(["event":"writer_finished","label":item.label,"output":item.recorder.writer.outputURL.path,"frames":item.recorder.frames,"valid":problem == nil && item.recorder.frames>0 && item.recorder.writer.status == .completed,"wall_time":Date().timeIntervalSince1970])
  return problem
 }
 static func main() async {
     ...
     do {
      ...
      if generation != generations[label] {
       ...
       if let item=active.removeValue(forKey:label) {
        if let problem=await finish(item){throw Failure("Finalization failed: "+problem)}
       }
       if wanted {
      ...
      targetFailures.append(label+": "+String(describing:error))
  ...
  if !targetFailures.isEmpty {fatal=fatal ?? targetFailures.joined(separator:"; ")}
