...
class InputView:NSView {
 ...
 override func draw(_ dirtyRect:NSRect) {
  ...
  let lines=["Foreground mouse and keyboard probe", "Actual mouse clicks: \(clicks)  |  moves: \(moves)", "Actual key input: \(text)", "Blender A and B operate in separate full-screen Spaces.", "This window receives native system input during the test.", "Observed foreground PID: \(NSWorkspace.shared.frontmostApplication?.processIdentifier ?? -1)", "System cursor: \(Int(CGEvent(source:nil)!.location.x)), \(Int(CGEvent(source:nil)!.location.y))   Clipboard revision: \(NSPasteboard.general.changeCount)"]
...
let timer=Timer.scheduledTimer(withTimeInterval:0.01,repeats:true){ _ in
 ...
 if signature != last || Date().timeIntervalSince1970-lastHeartbeat>=0.5 {record(data);last=signature;lastHeartbeat=Date().timeIntervalSince1970;view.needsDisplay=true}
}
